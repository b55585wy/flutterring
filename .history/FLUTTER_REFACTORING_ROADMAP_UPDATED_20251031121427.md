...
### **问题清单**:
1. ❌ **测量时长无法选择** - 当前硬编码为60秒，无法调整
2. ❌ **开始测量后程序崩溃** - 需要排查具体崩溃原因
3. ❌ **重启后难以扫描到戒指** - 可能是戒指未广播或断连状态异常

**🎯 下一步优先级（按顺序）**:
1. **任务 1.5 - 修复测量功能的关键问题** ⭐⭐⭐（最高优先级）
   - 添加测量时长选择对话框
   - 排查并修复程序崩溃问题
   - 修复重启后扫描不到设备的问题
   - 改善连接状态管理

2. **任务 1.1 - 实现实时数据流** ⭐⭐⭐（最高优先级）
...
#### 📋 任务 1.5: 修复测量功能的关键问题 🚨 **新增 - 紧急**

**目标**: 修复当前测量功能的关键问题，确保基本功能可用。

**问题分析**:

**问题 1: 测量时长无法选择**
```dart
// 当前代码（measurement_page.dart:331）
await RingPlatform.startLiveMeasurement(duration: 60); // ← 硬编码60秒
```

**问题 2: 开始测量后程序崩溃**
可能原因：
- MainActivity.kt 中 `startLiveMeasurement` 方法未实现或实现有误
- BLE 数据解析时出现空指针异常
- EventChannel 发送数据格式错误
- 戒指端命令格式不正确
...
1. **添加测量时长选择功能** ⏳
   - [ ] 创建测量配置对话框（`_showMeasurementConfigDialog`）
   - [ ] 支持选择时长：30秒、60秒、120秒、300秒、自定义
   - [ ] 将选择的时长传递给 `startLiveMeasurement`
   
   ```dart
   // measurement_page.dart - 添加配置对话框
   Future<void> _showMeasurementConfigDialog() async {
     int? selectedDuration = await showDialog<int>(
       context: context,
       builder: (context) => AlertDialog(
         title: const Text('测量配置'),
         content: Column(
           ...
           children: [
             ...
             ListTile(
               title: const Text('2 分钟 (120秒)'),
               onTap: () => Navigator.pop(context, 120),
             ),
             ListTile(
               title: const Text('5 分钟 (300秒)'),
     ...
     if (selectedDuration != null) {
       await RingPlatform.startLiveMeasurement(duration: selectedDuration);
...
2. **排查并修复程序崩溃问题** ⏳
   ...
   **步骤 A: 检查 MainActivity.kt 实现**
   - [ ] 确认 `startLiveMeasurement` 方法是否已实现
   - [ ] 添加完整的 try-catch 异常处理
   - [ ] 添加详细日志输出
   
   ```kotlin
   // MainActivity.kt - 检查实现
   "startLiveMeasurement" -> {
     try {
       val duration = call.argument<Int>("duration") ?: 60
       Log.d(TAG, "Starting live measurement, duration: $duration seconds")
       ...
       // 调用 AAR 库方法
       bleManager?.startLiveMeasurement(duration)
       result.success(null)
       
       Log.d(TAG, "Live measurement started successfully")
     } catch (e: Exception) {
       Log.e(TAG, "Failed to start live measurement", e)
   ...
   **步骤 B: 添加崩溃日志收集**
   - [ ] 使用 Android Studio Logcat 查看崩溃堆栈
   - [ ] 在 Flutter 侧添加全局错误捕获
   ...
   **步骤 C: 检查 BLE 数据解析**
   - [ ] 确认 AAR 库的数据回调是否正确设置
   - [ ] 检查 `onCharacteristicChanged` 回调是否存在
   - [ ] 验证数据格式解析逻辑

3. **修复重启后扫描不到设备的问题** ⏳
   
   **步骤 A: 改善断连逻辑**
   - [ ] 在 MainActivity.kt 中添加完整的断连清理
   ...
   **步骤 B: 清除 BLE 缓存**
   - [ ] 在 BleManager 中添加 `refreshDeviceCache` 方法
   ...
   **步骤 C: 改善扫描逻辑**
   ...
   "scanDevices" -> {
     ...
     Handler(Looper.getMainLooper()).postDelayed({
       ...
       Handler(Looper.getMainLooper()).postDelayed({
         bluetoothAdapter?.bluetoothLeScanner?.stopScan(scanCallback)
         sendEvent("scanCompleted", emptyMap())
       }, 15000)
     }, 500)
               ...
               case ConnectionState.connected:
                 return Row(
                   children: [
                     ...
                     SizedBox(width: 8),
                     Text('已连接: ${name ?? address}'),
                   ],
                 );
               case ConnectionState.disconnected:
                 return Text('未连接', style: TextStyle(color: Colors.grey));
               case ConnectionState.error:
                 return Text('连接失败', style: TextStyle(color: Colors.red));
               default:
                 return Text('未知状态');
             }
           },
           orElse: () => const Text('未连接'),
         );
       },
     );
   }
...
**验收标准**:
- ✅ 点击"开始测量"按钮后，弹出时长选择对话框
- ✅ 可以选择不同的测量时长（30/60/120/300秒或自定义）
- ✅ 开始测量后程序不崩溃，能正常运行
- ✅ App 重启后，能够正常扫描并连接到戒指
- ✅ 连接状态实时显示在 UI 上

**调试建议**:
1. **使用 Android Studio Logcat** 实时查看日志
   ```
   过滤标签: OpenRing, BLE, Flutter
   ```

2. **添加详细日志**
   - MainActivity.kt: 每个方法调用前后都加日志
   - measurement_page.dart: 每个状态变化都加日志
...
|------|----------|--------|------|
| ✅ 阶段 0: 准备工作 | 3 天 | 3 | **已完成** |
| 🔴 阶段 1: Platform Channel 完善 | 5-6 天 | 5 | **进行中（有阻塞问题）** |
...
| ⚪ 阶段 7: 打包发布 | 1 天 | 1 | 待开始 |
| **V1.0 小计** | **21-29 天 (~4-6 周)** | **19** | **进度 ~20%** |

**注**: 阶段 1 新增任务 1.5（紧急修复），需优先完成后才能继续后续任务。
