# 🐛 程序崩溃问题修复指南

**问题**: 点击"开始测量"后程序崩溃  
**日期**: 2025-10-31

---

## 📋 问题根源分析

### 1. **主要问题：BLEService 状态检查缺失**

```kotlin
// MainActivity.kt 第 533-560 行
private fun startLiveMeasurement(duration: Int, result: MethodChannel.Result) {
    try {
        // ❌ 问题：没有检查 BLEService 是否正在运行
        // ❌ 问题：没有检查设备是否已连接
        isMeasuring = true
        sampleBuffer.clear()
        
        // ❌ 这里直接发送命令，可能导致崩溃
        BLEService.sendCmd(startCmd)
        
        result.success(null)
    } catch (e: Exception) {
        result.error("START_MEASUREMENT_ERROR", e.message, null)
    }
}
```

### 2. **可能的崩溃场景**

**场景 A**: BLEService 未启动
- 用户没有先连接设备就点击"开始测量"
- BLEService 没有运行，广播接收器未注册
- `BLEService.sendCmd()` 发送的广播没有接收者

**场景 B**: 设备未连接
- 设备虽然扫描到了，但连接失败
- BLEService 启动了，但 BluetoothGatt 为 null
- 发送命令时没有 GATT 连接

**场景 C**: 命令格式错误
- `0xC4` 命令可能不是正确的启动命令
- 设备不识别该命令，返回错误

---

## 🔧 修复方案

### 方案 1: 添加完整的状态检查（推荐）

修改 `MainActivity.kt` 的 `startLiveMeasurement` 方法：

```kotlin
private var isConnected = false  // 添加连接状态标志

private fun handleConnectionStateChange(state: Int) {
    val stateString = when (state) {
        BLEService.CONNECT_STATE_SUCCESS -> {
            isConnected = true  // ✅ 连接成功时设置标志
            "connected"
        }
        BLEService.CONNECT_STATE_DISCONNECTED -> {
            isConnected = false  // ✅ 断连时清除标志
            "disconnected"
        }
        BLEService.CONNECT_STATE_GATT_CONNECTING -> "connecting"
        else -> "unknown"
    }
    
    handler.post {
        sendEvent(mapOf(
            "type" to "connectionStateChanged",
            "state" to stateString,
            "statusCode" to state
        ))
    }
}

private fun startLiveMeasurement(duration: Int, result: MethodChannel.Result) {
    try {
        android.util.Log.d("OpenRing", "开始在线测量，时长: $duration 秒")
        
        // ✅ 步骤 1: 检查连接状态
        if (!isConnected) {
            android.util.Log.e("OpenRing", "设备未连接，无法开始测量")
            result.error("NOT_CONNECTED", "Device not connected", null)
            return
        }
        
        // ✅ 步骤 2: 检查 BLEService 是否运行
        if (!isServiceRunning(BLEService::class.java)) {
            android.util.Log.e("OpenRing", "BLEService 未运行")
            result.error("SERVICE_NOT_RUNNING", "BLE Service is not running", null)
            return
        }
        
        // ✅ 步骤 3: 设置测量状态
        isMeasuring = true
        sampleBuffer.clear()
        
        // ✅ 步骤 4: 发送启动测量命令
        // 注意：命令可能需要根据实际协议调整
        val startCmd = byteArrayOf(0xC4.toByte())
        BLEService.sendCmd(startCmd)
        
        android.util.Log.d("OpenRing", "已发送启动测量命令")
        
        // ✅ 步骤 5: 设置自动停止
        if (duration > 0) {
            handler.postDelayed({
                stopMeasurement(null)
            }, (duration * 1000).toLong())
        }
        
        result.success(null)
    } catch (e: Exception) {
        android.util.Log.e("OpenRing", "启动测量失败: ${e.message}", e)
        isMeasuring = false
        result.error("START_MEASUREMENT_ERROR", e.message, null)
    }
}

// 辅助方法：检查服务是否运行
private fun isServiceRunning(serviceClass: Class<*>): Boolean {
    val manager = getSystemService(Context.ACTIVITY_SERVICE) as android.app.ActivityManager
    @Suppress("DEPRECATION")
    for (service in manager.getRunningServices(Integer.MAX_VALUE)) {
        if (serviceClass.name == service.service.className) {
            return true
        }
    }
    return false
}
```

---

### 方案 2: 改善 UI 反馈（配合方案 1）

修改 `measurement_page.dart`，在开始测量前检查连接状态：

```dart
// 添加连接状态监听
bool _isConnected = false;
StreamSubscription<BleEvent>? _connectionSubscription;

@override
void initState() {
  super.initState();
  
  // 监听连接状态
  _connectionSubscription = RingPlatform.eventStream.listen((event) {
    event.maybeWhen(
      connectionStateChanged: (state, name, address) {
        setState(() {
          _isConnected = (state == ConnectionState.connected);
        });
      },
      orElse: () {},
    );
  });
}

// 修改开始测量按钮
Widget _buildControlButton(...) {
  return ElevatedButton(
    // ✅ 未连接时禁用按钮
    onPressed: (!_isRecording && _isConnected) ? () async {
      // 显示时长选择对话框
      final duration = await _showDurationDialog();
      if (duration != null) {
        try {
          await RingPlatform.startLiveMeasurement(duration: duration);
          setState(() => _isRecording = true);
        } catch (e) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('启动测量失败: $e'),
                backgroundColor: Colors.red,
              ),
            );
          }
        }
      }
    } : null,  // ✅ 未连接时禁用
    child: Text(_isConnected ? '开始测量' : '请先连接设备'),
  );
}

// 添加时长选择对话框
Future<int?> _showDurationDialog() async {
  return showDialog<int>(
    context: context,
    builder: (context) => AlertDialog(
      title: const Text('选择测量时长'),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          _buildDurationOption('30 秒', 30),
          _buildDurationOption('1 分钟', 60),
          _buildDurationOption('2 分钟', 120),
          _buildDurationOption('5 分钟', 300),
        ],
      ),
    ),
  );
}

Widget _buildDurationOption(String label, int seconds) {
  return ListTile(
    title: Text(label),
    onTap: () => Navigator.pop(context, seconds),
  );
}
```

---

### 方案 3: 验证命令是否正确

需要确认 `0xC4` 是否是正确的启动命令。让我们查看原项目的命令定义：

```bash
# 在原项目中搜索启动测量的命令
grep -r "0xC4\|0xC5" app/src/main/java/
```

如果命令不正确，可能需要使用 LmAPI 提供的方法：

```kotlin
// 替代方案：使用 LmAPI 启动测量
private fun startLiveMeasurement(duration: Int, result: MethodChannel.Result) {
    try {
        if (!isConnected) {
            result.error("NOT_CONNECTED", "Device not connected", null)
            return
        }
        
        // ✅ 使用 LmAPI 的方法（如果有）
        // LmAPI.startRealTimeData() // 需要确认方法名
        
        // 或者使用正确的命令格式
        val cmd = CMDUtils.createStartMeasurementCmd(duration)  // 假设有这样的工具类
        BLEService.sendCmd(cmd)
        
        isMeasuring = true
        result.success(null)
    } catch (e: Exception) {
        result.error("START_MEASUREMENT_ERROR", e.message, null)
    }
}
```

---

## 🔍 调试步骤

### 步骤 1: 查看崩溃日志

1. 在 Android Studio 中打开 Logcat
2. 设置过滤器：`OpenRing|AndroidRuntime|FATAL`
3. 点击"开始测量"按钮
4. 复制完整的崩溃堆栈信息

**预期看到的日志**：
```
E/AndroidRuntime: FATAL EXCEPTION: main
    Process: com.tsinghua.openring_flutter, PID: xxxxx
    java.lang.NullPointerException: ...
        at com.tsinghua.openring.utils.BLEService.sendCmd(BLEService.java:xxx)
        at com.tsinghua.openring_flutter.MainActivity.startLiveMeasurement(MainActivity.kt:544)
        ...
```

### 步骤 2: 添加详细日志

在 `MainActivity.kt` 中添加更多日志：

```kotlin
private fun startLiveMeasurement(duration: Int, result: MethodChannel.Result) {
    android.util.Log.d("OpenRing", "========== 开始测量 ==========")
    android.util.Log.d("OpenRing", "时长: $duration 秒")
    android.util.Log.d("OpenRing", "连接状态: $isConnected")
    android.util.Log.d("OpenRing", "测量状态: $isMeasuring")
    
    try {
        if (!isConnected) {
            android.util.Log.e("OpenRing", "❌ 设备未连接")
            result.error("NOT_CONNECTED", "Device not connected", null)
            return
        }
        
        android.util.Log.d("OpenRing", "✅ 设备已连接，准备发送命令")
        
        isMeasuring = true
        sampleBuffer.clear()
        
        val startCmd = byteArrayOf(0xC4.toByte())
        android.util.Log.d("OpenRing", "发送命令: ${startCmd.joinToString(" ") { "%02X".format(it) }}")
        
        BLEService.sendCmd(startCmd)
        
        android.util.Log.d("OpenRing", "✅ 命令已发送")
        
        result.success(null)
    } catch (e: Exception) {
        android.util.Log.e("OpenRing", "❌ 启动测量异常", e)
        android.util.Log.e("OpenRing", "异常类型: ${e.javaClass.name}")
        android.util.Log.e("OpenRing", "异常消息: ${e.message}")
        e.printStackTrace()
        
        isMeasuring = false
        result.error("START_MEASUREMENT_ERROR", e.message, e.stackTraceToString())
    }
}
```

### 步骤 3: 验证连接状态

在点击"开始测量"前，先确认：

1. **设备是否已连接**
   - 在主页点击"扫描设备"
   - 找到戒指并点击连接
   - 等待连接状态变为"已连接"

2. **BLEService 是否运行**
   ```kotlin
   // 添加日志查看
   android.util.Log.d("OpenRing", "BLEService 运行状态: ${isServiceRunning(BLEService::class.java)}")
   ```

3. **BluetoothGatt 是否已建立**
   ```kotlin
   // 在 BLEService 中添加状态检查
   android.util.Log.d("BLEService", "mBluetoothGatt: ${mBluetoothGatt != null}")
   ```

---

## 📝 完整修复清单

- [ ] 1. 在 `MainActivity.kt` 添加 `isConnected` 状态标志
- [ ] 2. 在 `handleConnectionStateChange` 中更新连接状态
- [ ] 3. 在 `startLiveMeasurement` 中添加连接状态检查
- [ ] 4. 添加 `isServiceRunning` 辅助方法
- [ ] 5. 在 `measurement_page.dart` 中添加连接状态监听
- [ ] 6. 禁用未连接时的"开始测量"按钮
- [ ] 7. 添加测量时长选择对话框
- [ ] 8. 添加详细的日志输出
- [ ] 9. 测试完整流程：扫描 → 连接 → 测量
- [ ] 10. 验证崩溃是否修复

---

## ✅ 测试步骤

1. **编译并运行应用**
   ```bash
   cd openring_flutter
   flutter run
   ```

2. **测试流程**
   - 打开应用
   - 点击"扫描设备"
   - 选择戒指并连接
   - 等待连接成功提示
   - 进入测量页面
   - 点击"开始测量"
   - 选择时长
   - 观察是否还会崩溃

3. **查看日志**
   ```bash
   adb logcat | grep -E "OpenRing|BLEService|Flutter"
   ```

---

## 🚨 如果问题仍然存在

如果修复后仍然崩溃，请提供以下信息：

1. **完整的崩溃堆栈**（从 Logcat）
2. **点击"开始测量"前的日志**（包含连接状态）
3. **BLEService 的运行状态**
4. **测试设备信息**（Android 版本、手机型号）

然后我们可以进一步分析问题。

---

**最后更新**: 2025-10-31









