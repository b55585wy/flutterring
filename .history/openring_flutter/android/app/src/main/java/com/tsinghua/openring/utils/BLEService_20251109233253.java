...
public class BLEService extends Service {
    ...
    // private boolean isReceiverRegistered = false; // 已移除配对相关功能
    ...
    private BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            if (status == 133) {
                ...
                // GATT_INSUF_AUTHORIZATION - 需要配对
                Logger.show("BLEService", "连接失败 status=8 (需要授权)，设备可能需要配对");
                BLEUtils.setConnecting(false);
                // 不主动发起配对，让用户决定是否要配对
                // 如果用户在系统配对对话框中选择配对，下次连接就会成功
            } else if (status != BluetoothGatt.GATT_SUCCESS) {
                Logger.show("BLEService", "连接失败 status=" + status + "，重置连接状态");
                BLEUtils.setConnecting(false);
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent == null) {
            ...
            // 不管是否HID模式，都直接连接，不主动发起配对
            Log.d("Jason", "isBonded:" + isBonded);
            Logger.show("BLEService", "直接连接，不主动配对");
            this.connect();
    ...
    void connect() {
        Logger.show("BLEService", "connect() called, isConnecting=" + BLEUtils.isConnecting());
        if (!BLEUtils.isConnecting()) {
            BLEUtils.setConnecting(true);
            Logger.show("BLEService", "connect");
            if (mBluetoothGatt != null) {
                Logger.show("BLEService", "清理之前的连接");
                mBluetoothGatt.disconnect();
                mBluetoothGatt.close();
                mBluetoothGatt = null;
                try {
                    Thread.sleep(200); // 给一点时间确保清理完成
            ...
            // 检查是否已有连接
            if (mBluetoothGatt != null) {
                Logger.show("BLEService", "警告：检测到已有 GATT 连接，拒绝新的连接请求");
                BLEUtils.setConnecting(false);
                sendStatusChange(11); // 发送失败状态
                return;
            }
            
            if (VERSION.SDK_INT >= 23) {
                // 使用 TRANSPORT_LE (2) 明确指定低功耗蓝牙连接
                // autoConnect = false 防止自动重连
                mBluetoothGatt = mBluetoothDevice.connectGatt(this, false, this.mGattCallback, 2);
                Logger.show("BLEService", "使用 TRANSPORT_LE 模式连接");
            } else {
                mBluetoothGatt = mBluetoothDevice.connectGatt(this, false, this.mGattCallback);
            }
            
            // 注意：如果设备需要配对但未配对，系统会在连接时自动弹出配对对话框
            // 这是 Android 系统的安全机制，无法完全避免
            // 但我们不再主动调用 createBond()，让系统自己决定是否需要配对
            // 重试逻辑 + 异常捕获

            sendStatusChange(1);
        } else {
            Logger.show("BLEService", "connect() skipped because already connecting");
        }
