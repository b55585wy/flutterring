...
class MainActivity: FlutterActivity(), IResponseListener {
    ...
    // BLE 扫描回调 - 使用与原生代码相同的逻辑
    private val leScanCallback = BluetoothAdapter.LeScanCallback { device, rssi, scanRecord ->
        if (device == null || device.name.isNullOrEmpty()) {
            return@LeScanCallback
        }
        
        // 使用 LogicalApi 解析扫描数据（与原生代码相同）
        val bleDeviceInfo = LogicalApi.getBleDeviceInfoWhenBleScan(device, rssi, scanRecord, true)
    ...
    private fun scanDevices(result: MethodChannel.Result) {
        try {
            ...
            if (!adapter.isEnabled) {
                result.error("BLUETOOTH_DISABLED", "请先开启蓝牙", null)
                return
            }
            ...
            if (isScanning) {
                ...
                handler.post {
                    sendEvent(mapOf(
                        ...
                        "deviceCount" to scannedDevices.size
                    ))
                }
            }
            result.success(null)
