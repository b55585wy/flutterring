...
class MainActivity: FlutterActivity(), IResponseListener {
    ...
    private val leScanCallback = BluetoothAdapter.LeScanCallback { device, rssi, scanRecord ->
        ...
        // 使用 LogicalApi 解析扫描到的设备（与原生代码一致）
        val bleDeviceInfo: BleDeviceInfo? = LogicalApi.getBleDeviceInfoWhenBleScan(device, rssi, scanRecord, true)
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        methodChannel?.setMethodCallHandler { call, result ->
            when (call.method) {
                "scanDevices" -> {
                    // 扫描功能暂不实现 - 需要单独的扫描界面
                    result.error("NOT_IMPLEMENTED", "Scan feature not yet implemented", null)
                }
                "stopScan" -> {
                    result.success(null)
                }
