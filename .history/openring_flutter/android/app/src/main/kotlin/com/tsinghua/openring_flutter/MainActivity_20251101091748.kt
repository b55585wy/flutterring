    ...
    private fun startNewScan(adapter: BluetoothAdapter, result: MethodChannel.Result) {
        try {
            ...
            leScanCallback = BluetoothAdapter.LeScanCallback { device, rssi, scanRecord ->
                // ✅ 调试：打印所有扫描到的设备
                android.util.Log.d("OpenRing", "扫描到设备: name=${device?.name}, address=${device?.address}, rssi=$rssi")
                
                // 过滤：只显示有名称的设备
                if (device != null && !device.name.isNullOrEmpty()) {
                    android.util.Log.d("OpenRing", "设备有名称，检查兼容性: ${device.name}")
                    ...
                    if (bleDeviceInfo != null) {
                        android.util.Log.d("OpenRing", "✅ 发现兼容设备: ${device.name} (${device.address}) RSSI: $rssi")
                        handler.post {
                            sendEvent(mapOf(
                                "type" to "deviceFound",
                                "device" to mapOf(
                        ...
                        android.util.Log.w("OpenRing", "❌ 设备不兼容: ${device.name} (${device.address})")
                    }
                } else {
                    android.util.Log.d("OpenRing", "⚠️ 设备无名称: ${device?.address}")
                }
            }
