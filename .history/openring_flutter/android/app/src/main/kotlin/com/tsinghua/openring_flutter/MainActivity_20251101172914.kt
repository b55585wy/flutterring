...
class MainActivity: FlutterActivity(), IResponseListener {
    ...
    private fun syncConnectionState() {
        try {
            ...
            // 检查 GlobalParameterUtils 中是否有设备信息
            val device = GlobalParameterUtils.getInstance().device
            if (device != null) {
                android.util.Log.d("OpenRing", "🔄 发现缓存设备: ${device.name} - ${device.address}")
                ...
                try {
                    ...
                    val isBonded = bondedDevices?.any { it.address == device.address } ?: false
                    android.util.Log.d("OpenRing", "📱 设备是否已配对: $isBonded")
                    ...
                    currentConnectionState = "connected"
                    lastConnectedIso = Instant.now().toString()
                    android.util.Log.d("OpenRing", "✅ 状态已同步为 connected")
                } catch (e: SecurityException) {
                    android.util.Log.w("OpenRing", "⚠️ 缺少蓝牙权限: ${e.message}")
                    // 即使没有权限，也尝试恢复状态
                    currentDeviceName = device.name
                    currentDeviceAddress = device.address
                    isConnected = true
                ...
                android.util.Log.d("OpenRing", "ℹ️ 没有缓存的设备信息，状态保持为 disconnected")
            }
        } catch (e: Exception) {
