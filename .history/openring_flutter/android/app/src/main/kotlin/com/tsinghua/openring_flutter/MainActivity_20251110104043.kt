...
import com.tsinghua.openring.utils.BLEUtils
...
class MainActivity: FlutterActivity(), IResponseListener {
    ...
    private val customizeCmdListener = object : ICustomizeCmdListener {
        override fun cmdData(responseData: String?) {
            if (responseData.isNullOrBlank()) {
                android.util.Log.w("OpenRing", "收到空的自定义命令响应")
                return
            }
            try {
                val data = hexStringToByteArray(responseData)
                handleCustomCommandResponse(data)
            } catch (e: Exception) {
                android.util.Log.e("OpenRing", "解析自定义命令响应失败: ${e.message}", e)
            }
        }
    }
        ...
        try {
            ...
            if (device != null) {
                ...
                if (adapter.isEnabled) {
                    ...
                    // 如果正在连接或已连接其他设备，拒绝新的连接
                    if (isConnected || currentConnectionState == "connecting") {
                        android.util.Log.w("OpenRing", "已有活跃连接，拒绝新的连接请求")
                        result.error("ALREADY_CONNECTED", "请先断开当前连接", null)
                        return
                    }
                    
                    // 如果之前有连接，先停止旧的服务
                    if (currentConnectionState != "disconnected" || GlobalParameterUtils.getInstance().device != null) {
                        android.util.Log.d("OpenRing", "检测到之前的连接，先清理...")
                        val stopIntent = Intent(this, BLEService::class.java)
                        stopService(stopIntent)
                        Thread.sleep(500) // 给更多时间确保清理完成
                        GlobalParameterUtils.getInstance().device = null
                    }
                    
                    // 强制重置连接状态，确保能正常连接
                    BLEUtils.setConnecting(false)
                    android.util.Log.d("OpenRing", "已重置 BLEUtils.isConnecting = false")
                    
                    // 检查设备是否已配对
                    val isBonded = device.bondState == BluetoothDevice.BOND_BONDED
                    android.util.Log.d("OpenRing", "设备配对状态: isBonded=$isBonded")
                    
                    // 对于 BLE 设备，通常不需要配对即可连接
                    // 只有在连接失败且错误代码为 GATT_INSUF_AUTHORIZATION (8) 时才需要配对
                    // 因此这里不主动发起配对，让系统在需要时自动处理
                    ...
                    bleServiceIntent.putExtra("isBonded", isBonded)
    ...
    private fun disconnectDevice(result: MethodChannel.Result) {
        try {
            ...
            android.util.Log.d("OpenRing", "已清理保存的设备信息")
            ...
            android.util.Log.e("OpenRing", "断开连接失败: ${e.message}", e)
                ...
                if (device != null && !device.name.isNullOrEmpty()) {
                    ...
                    try {
                        ...
                        if (bleDeviceInfo != null) {
                            ...
                            // 调试模式：显示所有设备（包括被过滤的）
                            // 如果设备名称以 BCL 开头，视为戒指设备，正常显示
                            if (device.name.startsWith("BCL", ignoreCase = true)) {
                                android.util.Log.d(
                                    "OpenRing",
                                    "检测到 BCL 设备，作为戒指设备处理: ${device.name}"
                                )
                                handler.post {
                                    sendEvent(
                                        mapOf(
        ...
        when (state) {
            BLEService.CONNECT_STATE_SUCCESS -> {
                ...
                android.util.Log.d("OpenRing", "✅ 连接成功，isConnected = true")
                 ...
                 // 成功连接后，延时请求电量并获取固件版本
                scheduleBatteryAndVersionFetch()
                
                // 不再保存设备信息到 SharedPreferences，避免自动重连
                android.util.Log.d("OpenRing", "💾 跳过保存设备信息到 SharedPreferences")
            }
    ...
    private fun scheduleBatteryAndVersionFetch() {
        handler.postDelayed({
            try {
                requestBatteryLevel()
            } catch (e: Exception) {
                android.util.Log.e("OpenRing", "请求电量失败: ${e.message}", e)
            }
        }, 1000L)
        ...
        handler.postDelayed({
            try {
                ...
                android.util.Log.e("OpenRing", "获取固件版本失败: ${e.message}", e)
            }
        }, 1500L)
    }

    private fun requestBatteryLevel() {
        val frameId = generateRandomFrameId()
        val hexCommand = String.format("00%02X1200", frameId)
        val commandData = hexStringToByteArray(hexCommand)
        android.util.Log.d("OpenRing", "🔋 发送电量查询命令: $hexCommand")
        LmAPI.CUSTOMIZE_CMD(commandData, customizeCmdListener)
    }

    private fun handleCustomCommandResponse(data: ByteArray) {
        if (data.size < 4) {
            android.util.Log.w("OpenRing", "自定义命令响应长度不足: ${data.size}")
            return
        }

        val frameType = data[0].toInt() and 0xFF
        val cmd = data[2].toInt() and 0xFF
        val subcmd = data[3].toInt() and 0xFF
            ...
            when (cmd) {
                0x12 -> {
                    if (subcmd == 0x00) {
                        ...
                        android.util.Log.d("OpenRing", "忽略未处理的电量子命令: 0x${subcmd.toString(16)}")
                    }
                }
                else -> {
                    android.util.Log.d("OpenRing", "忽略未处理的自定义命令: cmd=0x${cmd.toString(16)}, subcmd=0x${subcmd.toString(16)}")
                }
            }
        }
    }

    private fun handleBatteryLevelResponse(data: ByteArray) {
        if (data.size < 5) {
            android.util.Log.w("OpenRing", "电量响应长度不足: ${data.size}")
            return
        }

        val level = data[4].toInt() and 0xFF
        currentBatteryLevel = level
        android.util.Log.d("OpenRing", "🔋 电池电量: $level%")

        handler.post {
            sendEvent(
                mapOf(
                    "type" to "batteryLevelUpdated",
                    "data" to mapOf(
                        "level" to level
                    )
                )
    ...
    private fun syncConnectionState() {
        try {
            ...
            if (device != null) {
                ...
                // 不再从 SharedPreferences 恢复设备信息
                android.util.Log.d("OpenRing", "ℹ️ 跳过从 SharedPreferences 恢复设备信息")
            }
    ...
    private fun buildConnectedDeviceInfo(): Map<String, Any?>? {
        ...
        currentBatteryLevel?.let { info["battery"] = it }
        return info
    }
    ...
    private fun generateRandomFrameId(): Int = random.nextInt(256)
