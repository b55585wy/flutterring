...
class DeviceConnectionController
  ...
  DeviceConnectionController({
    ...
    Future<int?> Function()? getBatteryLevel,
        ...
        _getConnectedDevice = getConnectedDevice ?? RingPlatform.getConnectedDevice,
  ...
  Future<void> _refreshDeviceInfo() async {
    try {
      ...
      if (info != null) {
        final enriched = battery != null
            ? info.copyWith(batteryLevel: battery)
            : info;
        state = state.copyWith(
          deviceInfo: enriched,
...
class DeviceConnectionState {
  ...
  DeviceConnectionState copyWith({
    ...
    return DeviceConnectionState(
      ...
      deviceName: deviceName == _noChange
          ? this.deviceName
          : deviceName as String?,
      address:
          address == _noChange ? this.address : address as String?,
      deviceInfo: deviceInfo == _noChange
          ? this.deviceInfo
          : deviceInfo as DeviceInfo?,
      isBusy: isBusy ?? this.isBusy,
      lastError: lastError == _noChange
          ? this.lastError
          : lastError as String?,
    );
  }
