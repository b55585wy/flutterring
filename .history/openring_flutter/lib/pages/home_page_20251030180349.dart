...
class _DashboardPageState extends ConsumerState<DashboardPage> {
  ...
  void _listenToBleEvents() {
    RingPlatform.eventStream.listen((event) {
      final bleEvent = BleEvent.fromMap(event);
      
      switch (bleEvent.type) {
        case BleEventType.deviceFound:
          final device = bleEvent.data['device'] as Map;
          setState(() {
            _foundDevices.add({
        ...
        case BleEventType.scanCompleted:
          setState(() {
            _isScanning = false;
          });
          break;
          
        case BleEventType.connectionStateChanged:
          final state = bleEvent.data['state'] as String;
          setState(() {
  ...
  Future<void> _connectDevice(String macAddress) async {
    try {
      ...
      if (mounted) {
        Navigator.pop(context); // 关闭设备列表对话框
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildQuickStat('心率', '--', 'BPM', Icons.favorite),
                      const SizedBox(width: 20),
                      _buildQuickStat('呼吸', '--', 'RPM', Icons.air),
                    ],
                  ),
                ],
              ),
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: (_isConnected ? const Color(0xFF10B981) : const Color(0xFFEF4444)).withOpacity(0.1),
                        borderRadius: BorderRadius.circular(12),
                      ),
                      child: Icon(
