...
class _DashboardPageState extends ConsumerState<DashboardPage> {
  ...
  void initState() {
    ...
    ref.listen<BleScanState>(bleScanControllerProvider, (previous, next) {
      if (next.errorMessage != null && previous?.errorMessage != next.errorMessage && mounted) {
    ...
    ref.listen<DeviceConnectionState>(
        deviceConnectionControllerProvider, (previous, next) {
      if (next.lastError != null && previous?.lastError != next.lastError && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _listenToBleEvents() {
    ...
    _bleSubscription = RingPlatform.eventStream.listen((event) {
      print('🔵 Flutter Home: 收到事件 ${event.runtimeType}');
      print('🔵 Flutter Home: 收到事件');
      event.when(
        deviceFound: (name, address, rssi) {
          print(
              '🔵 Flutter Home: deviceFound 回调 -> name=$name, address=$address, rssi=$rssi, mounted=$mounted, deviceSheet=${_deviceSheetSetState != null ? "有效" : "null"}');
          print(
