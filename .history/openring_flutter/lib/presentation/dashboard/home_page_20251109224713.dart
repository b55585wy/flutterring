...
class _DashboardPageState extends ConsumerState<DashboardPage> {
  ...
  void initState() {
    super.initState();
    _listenToBleEvents();
    _checkForSavedDevice();
  }

  Future<void> _checkForSavedDevice() async {
    // 检查是否需要记住设备
    final rememberDevice = await DeviceStorage.getRememberDevice();
    if (!rememberDevice) return;

    // 获取上次连接的设备
    final lastDevice = await DeviceStorage.getLastDevice();
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('是否连接到上次的设备 ${lastDevice.name}？'),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              DeviceStorage.clearLastDevice(); // 用户拒绝，清除记忆
            },
            child: const Text('取消'),
          ),
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              _connectDevice(lastDevice.address);
            },
            child: const Text('连接'),
          ),
        ],
