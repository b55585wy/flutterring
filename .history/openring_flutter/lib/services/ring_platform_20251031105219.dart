...
/// Platform Channel 接口
class RingPlatform {
  static const MethodChannel _methodChannel = MethodChannel('com.tsinghua.openring/ble_method');
  static const EventChannel _eventChannel = EventChannel('com.tsinghua.openring/ble_event');
  ...
  static Stream<BleEvent> get eventStream {
    return _eventChannel.receiveBroadcastStream().map((event) {
      ...
      switch (type) {
        ...
        case 'sampleBatch':
          final data = Map<String, dynamic>.from(map['data'] as Map);
          final samplesJson = (data['samples'] as List).cast<Map<String, dynamic>>();
          final samples = samplesJson.map((json) => Sample.fromJson(json)).toList();
        ...
        case 'fileListReceived':
          final data = Map<String, dynamic>.from(map['data'] as Map);
          final filesJson = (data['files'] as List).cast<Map<String, dynamic>>();
          final files = filesJson.map((json) => RingFile.fromJson(json)).toList();
  ...
  static Future<void> connectDevice(String macAddress) async {
    try {
      await _methodChannel.invokeMethod('connectDevice', {'macAddress': macAddress});
    } on PlatformException catch (e) {
  ...
  static Future<void> startLiveMeasurement({int duration = 60}) async {
    try {
      await _methodChannel.invokeMethod('startLiveMeasurement', {'duration': duration});
    } on PlatformException catch (e) {
