...
- [x] 真机测试：扫描 → 连接 → 断开 → 重新扫描，确保流程稳定。（2025-11-05 已走查）
- [x] 记录测试流程与结果，写入 `research/testing_logs.md`。（2025-11-03 自动化用例，2025-11-05 真机补充）
- [ ] 更新 `docs/software_architecture.md` 的 “M1 Scope” 小节（若需要）。
- [ ] 会议纪要：在 `meetings/` 文件更新执行情况。

### WP6 Measurement 控制器与日志统一（Plan）

- [ ] 实现 `measurement_controller.dart`（心率/呼吸/样本累积）。
- [ ] MeasurementPage 改造为 ConsumerWidget，使用 controller 状态。
- [ ] 建立统一日志工具（替换调试 print）。
- [ ] 更新测试：measurement 控制器单测、UI smoke 测试。