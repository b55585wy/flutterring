# 🎯 科研版当前行动清单

**日期**: 2025-10-30  
**目标**: 将当前 UI 调整为科研版定位

---

## 📋 立即调整项（本周）

### 1️⃣ **主页 (home_page.dart) - 移除装饰性元素**

#### **移除:**
```dart
// ❌ 删除渐变欢迎卡片
Container(
  decoration: BoxDecoration(
    gradient: LinearGradient(...),
  ),
  child: Column(
    children: [
      Text('早上好！'), // ← 不需要
      Text('开始今天的健康监测'), // ← 太消费化
    ],
  ),
)
```

#### **替换为:**
```dart
// ✅ 设备状态信息卡片
Card(
  child: Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('设备状态', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
      SizedBox(height: 12),
      
      // 设备信息
      _buildInfoRow('设备名称', 'Ring-A1B2C3'),
      _buildInfoRow('MAC 地址', '00:11:22:33:44:55'),
      _buildInfoRow('固件版本', 'v1.0.81'),
      _buildInfoRow('电池电量', '78%'),
      _buildInfoRow('连接状态', isConnected ? '已连接' : '未连接',
        valueColor: isConnected ? Colors.green : Colors.red),
    ],
  ),
)

Widget _buildInfoRow(String label, String value, {Color? valueColor}) {
  return Padding(
    padding: const EdgeInsets.symmetric(vertical: 4),
    child: Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: TextStyle(color: Colors.grey[600])),
        Text(value, style: TextStyle(
          fontWeight: FontWeight.w500,
          color: valueColor,
        )),
      ],
    ),
  );
}
```

#### **添加: 最近实验列表**
```dart
Card(
  child: Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('最近实验', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
      SizedBox(height: 12),
      
      ListView.separated(
        shrinkWrap: true,
        physics: NeverScrollableScrollPhysics(),
        itemCount: 5,
        separatorBuilder: (_, __) => Divider(),
        itemBuilder: (context, index) {
          return ListTile(
            dense: true,
            leading: Icon(Icons.file_present, color: Colors.blue),
            title: Text('2025-10-30 14:23:15'),
            subtitle: Text('时长: 5:32 | 大小: 2.3 MB'),
            trailing: Icon(Icons.chevron_right),
          );
        },
      ),
    ],
  ),
)
```

---

### 2️⃣ **测量页 (measurement_page.dart) - 专业化调整**

#### **移除:**
```dart
// ❌ 删除心跳动画
ScaleTransition(
  scale: _heartbeatAnimation,
  child: Icon(Icons.favorite, color: Colors.red, size: 32),
)
```

#### **替换为:**
```dart
// ✅ 静态图标 + 数据质量指示器
Row(
  children: [
    Icon(Icons.favorite, color: Colors.red, size: 24),
    SizedBox(width: 8),
    Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('72 BPM', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
        Text('置信度: 95%', style: TextStyle(fontSize: 12, color: Colors.grey)),
      ],
    ),
  ],
)
```

#### **添加: 通道选择器**
```dart
// 在波形图上方添加
Wrap(
  spacing: 8,
  children: [
    FilterChip(
      label: Text('PPG Green'),
      selected: _selectedChannels.contains('ppg_green'),
      onSelected: (selected) {
        setState(() {
          if (selected) {
            _selectedChannels.add('ppg_green');
          } else {
            _selectedChannels.remove('ppg_green');
          }
        });
      },
      selectedColor: Colors.green.withOpacity(0.3),
    ),
    FilterChip(
      label: Text('PPG Red'),
      selected: _selectedChannels.contains('ppg_red'),
      onSelected: (selected) { /* ... */ },
      selectedColor: Colors.red.withOpacity(0.3),
    ),
    FilterChip(
      label: Text('PPG IR'),
      selected: _selectedChannels.contains('ppg_ir'),
      onSelected: (selected) { /* ... */ },
      selectedColor: Colors.orange.withOpacity(0.3),
    ),
    FilterChip(label: Text('ACC X'), /* ... */),
    FilterChip(label: Text('ACC Y'), /* ... */),
    FilterChip(label: Text('ACC Z'), /* ... */),
    // ... 其他通道
  ],
)
```

#### **添加: 实时统计面板**
```dart
// 在生理指标下方添加
Card(
  color: Colors.grey[100],
  child: Padding(
    padding: EdgeInsets.all(12),
    child: Row(
      mainAxisAlignment: MainAxisAlignment.spaceAround,
      children: [
        _buildStatItem('采样率', '25 Hz'),
        _buildStatItem('已采样', '2048'),
        _buildStatItem('时长', '01:22'),
        _buildStatItem('丢包', '0.1%'),
      ],
    ),
  ),
)

Widget _buildStatItem(String label, String value) {
  return Column(
    children: [
      Text(value, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      Text(label, style: TextStyle(fontSize: 12, color: Colors.grey[600])),
    ],
  );
}
```

#### **添加: 波形控制工具栏**
```dart
Row(
  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
  children: [
    IconButton(
      icon: Icon(Icons.zoom_in),
      onPressed: () {/* 缩放波形 */},
      tooltip: 'Y轴放大',
    ),
    IconButton(
      icon: Icon(Icons.zoom_out),
      onPressed: () {/* 缩放波形 */},
      tooltip: 'Y轴缩小',
    ),
    IconButton(
      icon: Icon(Icons.pause),
      onPressed: () {/* 暂停刷新 */},
      tooltip: '暂停刷新',
    ),
    IconButton(
      icon: Icon(Icons.bookmark_add),
      onPressed: () {/* 添加标注 */},
      tooltip: '添加标注',
    ),
  ],
)
```

---

### 3️⃣ **历史页 (history_page.dart) - 详细信息**

#### **增强记录列表:**
```dart
ListTile(
  leading: Icon(Icons.file_present),
  title: Text('2025-10-30 14:23:15'),
  subtitle: Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('时长: 5:32 | 采样: 8320 点'),
      Text('HR: 68-76 BPM | RR: 14-18 RPM'),
      Text('信号质量: 优秀 (98%) | 文件: 2.3 MB'),
    ],
  ),
  trailing: PopupMenuButton(
    itemBuilder: (context) => [
      PopupMenuItem(child: Text('查看详情')),
      PopupMenuItem(child: Text('导出 CSV')),
      PopupMenuItem(child: Text('删除')),
    ],
  ),
)
```

#### **添加批量操作:**
```dart
// 顶部添加
Row(
  children: [
    Checkbox(
      value: _selectAll,
      onChanged: (value) {
        setState(() {
          _selectAll = value ?? false;
          // 全选/取消全选
        });
      },
    ),
    Text('全选'),
    Spacer(),
    if (_selectedRecords.isNotEmpty) ...[
      TextButton.icon(
        icon: Icon(Icons.file_download),
        label: Text('批量导出 (${_selectedRecords.length})'),
        onPressed: () {/* 批量导出 */},
      ),
      TextButton.icon(
        icon: Icon(Icons.delete),
        label: Text('批量删除'),
        onPressed: () {/* 批量删除 */},
        style: TextButton.styleFrom(foregroundColor: Colors.red),
      ),
    ],
  ],
)
```

---

### 4️⃣ **设置页 (settings_page.dart) - 科研专用设置**

#### **添加算法参数分组:**
```dart
_buildSettingGroup(
  '算法参数',
  [
    ListTile(
      leading: Icon(Icons.tune, color: Colors.blue),
      title: Text('心率计算窗口'),
      subtitle: Text('当前: 8 秒'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        // 打开参数调节对话框
        _showParameterDialog('HR Window', 8, 1, 30);
      },
    ),
    ListTile(
      leading: Icon(Icons.tune, color: Colors.green),
      title: Text('呼吸率计算窗口'),
      subtitle: Text('当前: 30 秒'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        _showParameterDialog('RR Window', 30, 10, 60);
      },
    ),
    ListTile(
      leading: Icon(Icons.tune, color: Colors.orange),
      title: Text('峰值检测阈值'),
      subtitle: Text('当前: 0.6'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        _showParameterDialog('Peak Threshold', 0.6, 0.1, 1.0);
      },
    ),
  ],
),
```

#### **添加导出设置分组:**
```dart
_buildSettingGroup(
  '导出设置',
  [
    ListTile(
      leading: Icon(Icons.text_fields, color: Colors.purple),
      title: Text('CSV 分隔符'),
      subtitle: Text('当前: 逗号'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        // 选择分隔符（逗号/制表符/分号）
      },
    ),
    ListTile(
      leading: Icon(Icons.access_time, color: Colors.teal),
      title: Text('时间戳格式'),
      subtitle: Text('当前: Unix (毫秒)'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        // 选择时间戳格式
      },
    ),
    ListTile(
      leading: Icon(Icons.format_list_numbered, color: Colors.indigo),
      title: Text('数值精度'),
      subtitle: Text('当前: 保留 3 位小数'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        // 选择小数位数
      },
    ),
  ],
),
```

#### **添加实验管理分组:**
```dart
_buildSettingGroup(
  '实验管理',
  [
    ListTile(
      leading: Icon(Icons.person, color: Colors.green),
      title: Text('受试者信息模板'),
      subtitle: Text('设置默认字段'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        // 打开模板编辑器
      },
    ),
    ListTile(
      leading: Icon(Icons.science, color: Colors.blue),
      title: Text('实验项目管理'),
      subtitle: Text('创建和管理实验'),
      trailing: Icon(Icons.chevron_right),
      onTap: () {
        // 打开项目管理页面
      },
    ),
  ],
),
```

---

## 🎨 UI 通用调整

### **减小间距，增加信息密度**

```dart
// 全局调整 padding
- const EdgeInsets.all(24)
+ const EdgeInsets.all(12)

- const EdgeInsets.symmetric(horizontal: 20, vertical: 16)
+ const EdgeInsets.symmetric(horizontal: 12, vertical: 8)
```

### **使用更专业的配色**

```dart
// 减少渐变，使用纯色
- Container(decoration: BoxDecoration(gradient: LinearGradient(...)))
+ Container(color: Colors.grey[50])

// 减少圆角
- BorderRadius.circular(20)
+ BorderRadius.circular(8)

// 减少阴影
- boxShadow: [BoxShadow(blurRadius: 20, ...)]
+ boxShadow: [BoxShadow(blurRadius: 4, spreadRadius: 1, ...)]
```

---

## 📊 下周开发（新功能）

### 1. **CSV 导出功能**
```dart
// lib/services/csv_exporter.dart
class CSVExporter {
  Future<String> exportMeasurement(Measurement measurement) async {
    // 生成 CSV 文件
    // 包含：时间戳、12 通道数据、HR、RR、质量
  }
  
  Future<void> exportBatch(List<Measurement> measurements) async {
    // 批量导出
  }
}
```

### 2. **通道数据管理**
```dart
// lib/models/channel_data.dart
enum ChannelType {
  ppgGreen, ppgRed, ppgIr,
  accX, accY, accZ,
  gyroX, gyroY, gyroZ,
  temp0, temp1, temp2,
}

class ChannelBuffer {
  final ChannelType type;
  final List<double> data;
  final int maxSize;
  
  void addSample(double value) {
    data.add(value);
    if (data.length > maxSize) {
      data.removeAt(0);
    }
  }
}
```

### 3. **波形标注功能**
```dart
// lib/models/annotation.dart
class WaveformAnnotation {
  final DateTime timestamp;
  final int sampleIndex;
  final String label;
  final AnnotationType type;
  final String? notes;
}

enum AnnotationType {
  eventStart,
  eventEnd,
  anomaly,
  manual,
}
```

---

## ✅ 验收标准

### **本周完成后应该看到:**

1. ✅ 主页显示设备详细信息（MAC、版本、电量）
2. ✅ 主页显示最近 5 次实验记录
3. ✅ 测量页移除心跳动画，改为静态 + 置信度
4. ✅ 测量页有通道选择器（至少 6 个通道）
5. ✅ 测量页有实时统计面板（采样率、数量、时长、丢包）
6. ✅ 测量页有波形控制工具栏（缩放、暂停、标注按钮）
7. ✅ 历史页显示详细信息（采样点数、HR/RR 范围、文件大小）
8. ✅ 历史页有批量操作（全选、批量导出、批量删除）
9. ✅ 设置页有算法参数分组（3 个参数）
10. ✅ 设置页有导出设置分组（3 个选项）
11. ✅ 设置页有实验管理分组（2 个功能）

### **整体效果:**
- 信息密度提升 50%（更多文本信息，更少装饰）
- 专业感增强（技术参数可见）
- 工具性突出（控制按钮、批量操作）

---

## 🚀 开始调整

### **建议顺序:**

1. **今天**: 调整主页（移除欢迎卡片，添加设备状态）
2. **明天**: 调整测量页（移除动画，添加统计面板）
3. **后天**: 调整测量页（添加通道选择器和工具栏）
4. **周四**: 调整历史页（详细信息和批量操作）
5. **周五**: 调整设置页（科研专用设置）

---

**从消费级 UI 升级为科研级工具！** 🔬

